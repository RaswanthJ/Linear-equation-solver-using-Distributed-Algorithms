...
def send_init_message(ip, port, node_id,iteration_number,neighbors,alpha,iter,nodes,beta,gamma,delta,gaps):
...
def send_messages_to_node(ip, port, node_id, neighbors, nodes,iteration_number,alpha,iter,beta,gamma,delta,gaps):
    send_init_message(ip, port, node_id, iteration_number,neighbors,alpha,iter,nodes,beta,gamma,delta,gaps)
...
if __name__ == "__main__":
    ...
    iteration_number = 500000  #Its upto the user to decide the number of iterations. More the iterations accurate are the results
    ...
    for node in nodes:
        thread = threading.Thread(target=send_messages_to_node, args=(node[0], node[1], node[2], neighbors, nodes,iteration_number,alpha,iter,beta,gamma,delta,gaps))
